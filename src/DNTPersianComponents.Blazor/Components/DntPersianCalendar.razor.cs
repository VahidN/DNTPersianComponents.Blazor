...
namespace DNTPersianComponents.Blazor
{
    ...
    /// </summary>
    public partial class DntPersianCalendar : ComponentBase
    {
        ...
        private IEnumerable<string> PersianDayNamesOfWeek =>
            UseShortPersianDayNamesOfWeek ?
                PersianCulture.ShortPersianDayWeekNames :
                PersianCulture.PersianDayNamesOfWeek;
        ...
        /// </summary>
        [Parameter] public Action<int>? SelectedMonthChanged { set; get; }
        ...
        /// </summary>
        [Parameter] public bool ShowYearMonthDropdowns { get; set; } = true;

        /// <summary>
        /// Should we display the related hoidays of the current month?
        /// </summary>
        [Parameter] public bool ShowHolidays { get; set; }

        /// <summary>
        /// Holiday's text custom css class. Its default value is `text-danger`.
        /// </summary>
        [Parameter] public string HolidayTextCss { set; get; } = "text-danger";

        /// <summary>
        /// Returns the list of related events of the given Persian month.
        /// </summary>
        [Parameter] public Func<EventDate, Task<IReadOnlyList<PersianDayEvents>>>? PerpareEventsList { set; get; }
        ...
        internal Task OnSelectedMonthChangedAsync(ChangeEventArgs args)
        {
        ...
        internal Task OnSelectedYearChangedAsync(ChangeEventArgs args)
        {
            var value = args.Value?.ToString();
        ...
        internal void OnDayClicked(PersianDay day)
        {
            DayClicked?.Invoke(day);
        }

        internal async Task CreateCalendarAsync()
        {
            try
            {
                IsLoading = true;
                ...
                if (PerpareEventsList is not null)
                {
                    _currentEvents = await PerpareEventsList.Invoke(new(SelectedYear, SelectedMonth));
                }
            }
            finally
            {
        ...
        internal Task ShowPreviousMonthAsync()
        {
            if (SelectedMonth == 1)
            {
                SelectedMonth = 12;
        ...
        internal Task ShowNextMonthAsync()
        {
            if (SelectedMonth == 12)
            {
                SelectedMonth = 1;
        ...
        internal Task ShowPreviousYearAsync()
        {
        ...
        internal Task ShowNextYearAsync()
        {
        ...
        internal string GetTodayHighlighterCss(PersianDay? day)
        {
            return day != null && _today.Equals(day) ? TodayHighlightCss : string.Empty;
        }

        internal string GetHolidayTextCss(PersianDay? day, int cellNumber)
        {
            if (day is null)
            {
                return string.Empty;
        ...
        internal IReadOnlyList<DayEvent> GetDayEvents(PersianDay? day)
        {
        ...
        internal static string GetTimeRangeText(DayEvent? dayEvent)
        {
